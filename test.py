...
from core.evaluation import eval_flow_avg, load_gt_flow_kitti, flowlib
from core.dataset import KITTI_2012, KITTI_2015
# from core.evaluation.flowlib import read_flow_png, flow_to_image
import os
import sys
...
def test_kitti_2012(cfg, model, gt_flows, noc_masks):
    ...
    eval_flow_res = eval_flow_avg(
        gt_flows, noc_masks, flow_list, cfg, write_img=False)
...
def test_kitti_2015(cfg, model, gt_flows, noc_masks, gt_masks, depth_save_dir=None):
    ...
    # pdb.set_trace()
    eval_flow_res = eval_flow_avg(
        gt_flows, noc_masks, pred_flow_list, cfg, moving_masks=gt_masks, write_img=False)
    print('CONFIG: {0}, mode: {1}'.format(cfg.config_file, cfg.mode))
...
def test_nyu(cfg, model, test_images, test_gt_depths):
    ...
    print('Test nyu depth on '+str(leng) +
          ' images. Using depth model in '+cfg.model_dir)
    pred_disp_list = []
    crop_imgs = []
    crop_gt_depths = []
    for i in range(leng):
        ...
        img = np.transpose(cv2.resize(np.transpose(
            img_crop, [1, 2, 0]), (cfg.img_hw[1], cfg.img_hw[0])), [2, 0, 1])
...
def test_folder(folder, model, training_hw, save_dir='./results'):
    img_paths = sorted(os.listdir(folder))
    for path in img_paths:
        img = cv2.imread(os.path.join(folder, path))
        h, w = img.shape[0:2]
        img1, img2 = img[:int(h/2), :, :], img[int(h/2):, :, :]
        img1_resized = np.transpose(cv2.resize(
            img1, (training_hw[1], training_hw[0])), [2, 0, 1])
        img2_resized = np.transpose(cv2.resize(
            img2, (training_hw[1], training_hw[0])), [2, 0, 1])
        img1_t = torch.from_numpy(
            img1_resized).float().cuda().unsqueeze(0) / 255.0
        img2_t = torch.from_numpy(
            img2_resized).float().cuda().unsqueeze(0) / 255.0
        flow = model.inference_flow(img1_t, img2_t)
        flow = flow[0].detach().cpu().numpy()
        flow = flow.transpose(1, 2, 0)
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        cv2.imwrite(
            os.path.join(save_dir, path), flowlib.flow_to_image(flow))
        print(path)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument(
        '--mode', type=str, default='depth', help='mode for testing.')
    ...
    arg_parser.add_argument('--result_dir', type=str,
                            default=None, help='directory for saving predictions')
    ...
    if args.task == 'kitti_depth':
        ...
        gt_flows_2015, noc_masks_2015 = load_gt_flow_kitti(
            cfg_new.gt_2015_dir, 'kitti_2015')
        gt_masks_2015 = load_gt_mask(cfg_new.gt_2015_dir)
        flow_res = test_kitti_2015(
            cfg_new, model, gt_flows_2015, noc_masks_2015, gt_masks_2015)
        ...
        test_single_image(args.image_path, model,
                          training_hw=cfg['img_hw'], save_dir=args.result_dir)
    elif args.task == 'rs':
        test_folder(args.image_path, model,
                          training_hw=cfg['img_hw'], save_dir=args.result_dir)
